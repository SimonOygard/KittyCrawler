...
// ── Deck-oppsett ──────────────────────────────────────────────────
        ...
        private void OnSlotClicked(InputEvent inputEvent, Slot.SlotPosition position)
        {
            ...
            if (success)
            {
                if (!_abilityResolver.NeedsTarget(_selectedCard.CardData))
                {
                    var ability = _selectedCard.CardData.Ability;
                    ...
                    GetTree().CreateTimer(0.05f).Timeout += () =>
                    {
                        switch (ability)
                        {
                            case CardData.AbilityType.AllEnemyMinusStat:
                                FlashAllOccupiedSlots(false, new Color(1f, 0.2f, 0.2f, 1f)); // rød på fiendens slots
                                break;
                            case CardData.AbilityType.AllAllyPlusStat:
                                FlashAllOccupiedSlots(true, new Color(0.2f, 1f, 0.2f, 1f)); // grønn på egne slots
                                break;
                        }
                    };
                    UpdateUI();
        ...
        private void OnTurnChanged(GameManager.TurnOwner turn)
        {
            ...
            if (turn == GameManager.TurnOwner.Enemy)
            {
                ...
                float delay = isVeryFirstTurn ? 3.7f : 0.4f;
        ...
        private void FlashAllOccupiedSlots(bool isPlayerSlots, Color color)
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                var panel = isPlayerSlots ? _playerSlotsContainer.GetChild<Panel>(i) : _enemySlotsContainer.GetChild<Panel>(i);
                foreach (Node child in panel.GetChildren())
                    if (child is CardVisual visual && !visual.IsQueuedForDeletion())
                        visual.FlashStatColor(color);
            }
        ...
        private void FlashSlotForCard(CardData card, bool isPlayer, Color color)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                {
                    var panel = isPlayer ? _playerSlotsContainer.GetChild<Panel>(i) : _enemySlotsContainer.GetChild<Panel>(i);
                    foreach (Node child in panel.GetChildren())
                        if (child is CardVisual visual && !visual.IsQueuedForDeletion())
                            visual.FlashStatColor(color);
                    return;
