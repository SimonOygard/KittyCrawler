...
public partial class MimicManager : Node2D
{
    ...
    private void AssignMimic()
    {
        ...
        int nextIndex = rng.RandiRange(0, chests.Count - 1);
        if (index == nextIndex)
        {
            nextIndex = (nextIndex + 1) % chests.Count; // Ensure a different chest is selected
        }
        chests[nextIndex].IsMimic = true;
